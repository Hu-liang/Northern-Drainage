...
<template>
  <div style="height:100%" class="publish-container">
    <group title="">
      <x-input title="备勤令标题" required v-model="title" name="title" placeholder="请输入通知标题" ></x-input>
    </group>
    <!-- <group title="">
      <x-input title="备勤令期号" required v-model="date" name="date" placeholder="请输入通知标题" ></x-input>
    </group> -->
    <group title="备勤令内容">
      <x-textarea required name="content" v-model="content" placeholder="请输入备勤令内容"></x-textarea>
    </group>
...
export default {
  ...
  methods: {
    ...
    submit() {
      ...
      // }
      if (!this.content) {
        this.showWarn("备勤令内容不能为空");
        return false;
      }
      axios.post('/preparation/order/save',{
